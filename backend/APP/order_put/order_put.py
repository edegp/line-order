...
from table_order.table_order_payment_order_info import (
...
def get_order_item_info(item):
    ...
    for item in order_items:
        if category_id is None:
            ...
            item_info_list = item_list_table_controller.get_item(category_id)
        elif category_id != item["categoryId"]:
            item_info_list = item_list_table_controller.get_item(item["categoryId"])
        put_order_items.append(
            get_item_info_item_id(item["itemId"], item["orderNum"], item_info_list)
        )
...
def update_payment_info(params, now):
    ...
    try:
        payment_order_table_controller.update_order(
            payment_id, params["userId"], payment_info["order"], payment_info["amount"]
        )
    except ClientError as e:
        if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
            logger.error(
...
def put_order(params):
    ...
    """
    now = datetime.datetime.now(gettz("Asia/Tokyo")).strftime("%Y/%m/%d %H:%M:%S")
...
def lambda_handler(event, context):
    ...
    return utils.create_success_response(
        json.dumps(payment_id, default=utils.decimal_to_int, ensure_ascii=False)
    )
